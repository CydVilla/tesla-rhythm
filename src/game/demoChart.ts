...
/** Convenience: build a tap (or hold, when `durationBeats` is given) note. */
function note(beat: number, lane: Lane, durationBeats?: number): ChartNote {
  const durationMs =
    durationBeats && durationBeats > 0 ? beatsToMs(durationBeats, DEMO_BPM) : undefined;
  return {
    id: makeNoteId("demo"),
 ...
 * without hand-writing dozens of notes. Each phrase ends on a 1.5-beat sustain
 * so hold notes show up regularly in the demo.
 */
...
function buildDemoNotes(): ChartNote[] {
  ...
  // 4-beat lead-in with a simple climbing intro, capped by a 2-beat sustain to
  // introduce the hold mechanic early.
  notes.push(note(4, 0), note(5, 1), note(6, 2), note(7, 3, 2));
  ...
  // A small chord-flavoured outro that finishes on a long held chord.
  const outro = 40;
  notes.push(
    note(outro + 0, 0),
    note(outro + 0, 4),
